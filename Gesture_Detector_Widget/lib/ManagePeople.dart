  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onScaleUpdate: (ScaleUpdateDetails e) {
        // 핀 아웃 설정 (두 손가락을 벌리는 동작) 이동작은 body 어디서 하든 상관없음
        if (e.scale > 2.0) addPerson(context);
      },
      onDoubleTap: () =>
          addPerson(context), // 사람추가 더블클릭시, 이동작은 body 어디서 하든 상관없음
      child: ListView(
        children: people
            .map((Map<String, dynamic> person) => GestureDetector(
                  ...
                  onLongPress: () {
                    deletePerson(
                        person, context); // 삭제기능 이 동작은 리스트 내에서 1초가 누르면 동작
                  },
                  ...
                  onHorizontalDragUpdate: (DragUpdateDetails e) {
                    // 드래그 시작점과 끝을 비교하여 왼쪽 드래그인지 오른쪽 드래그인지 판단
                    _swipeDirection =
